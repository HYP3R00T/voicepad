...
Package publication uses the manually dispatched **Release Packages** workflow. Selecting the package and bump type from the current `main` head is explicit release approval. The prepare job updates the version, changelog, lock, and export files on a deterministic release branch, opens a release pull request, authorizes and waits for that exact PR head's required `CI (uv)`, and then performs a normal exact-head merge. It authorizes only execution of the generated PR's CI; it does not approve the pull request, enable GitHub auto-merge, bypass protection, publish, tag, or push directly to `main`.

If preparation fails after creating a release PR, a new dispatch may reuse it only when merging current `main` into the branch produces the exact same full Git tree as a fresh preparation; conflicts or mismatches fail without overwriting the branch. After the protected merge succeeds, a separate job rebuilds the selected package from the merge commit, creates or validates its package tag, publishes with `uv publish --check-url`, and creates the GitHub release. Release `voicepad-core` before a dependent `voicepad` version. If tagging, publication, or release creation fails, rerun only the failed publish job so matching uploads are skipped safely.
