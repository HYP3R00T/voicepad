...
from voicepad_core.models import AuxiliaryArtifact, Model
...
def test_prepare_downloads_auxiliary_artifacts(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Preparation downloads required files supplied by a companion repository."""
    model = Model(
        "tiny",
        "owner/tiny",
        "test",
    ...
    auxiliary_calls: list[dict[str, object]] = []

    def download_snapshot(**kwargs: object) -> None:
        (Path(str(kwargs["local_dir"])) / "model.bin").write_bytes(b"model")

    def download_file(**kwargs: object) -> None:
        auxiliary_calls.append(kwargs)
        (Path(str(kwargs["local_dir"])) / "tokenizer.json").write_bytes(b"tokenizer")

    monkeypatch.setattr("huggingface_hub.snapshot_download", download_snapshot)
    monkeypatch.setattr("huggingface_hub.hf_hub_download", download_file)

    result = prepare_artifact(model, tmp_path)
