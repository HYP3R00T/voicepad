...
def deduplicate_overlap(
    ...
    prev_words = prev_text.split()[-_PREV_TAIL_WORDS:]
    ...
    if similarity >= _FULL_DUPLICATE_THRESHOLD:
        logger.debug(
    ...
    if len(overlap_words) >= _MIN_OVERLAP_WORDS_FOR_PARTIAL:
        lead = " ".join(overlap_words[:_PARTIAL_LEAD_WORDS])
